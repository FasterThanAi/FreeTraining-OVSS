...
four datasets. \emph{catch-all} is that class's share of ground truth;
\emph{discarded} is the share of real-class pixels assigned to it. The Potsdam row
was predicted from ground-truth masks alone and registered before the pipeline was
run.}
\label{tab:cmech}
\begin{tabular}{lrr}
\toprule
...
Per-class thresholds are everything available after the argmax, and the scale acts
\emph{at} it. That leaves the score the argmax reads, and we fitted three further
per-class corrections on the same tiles, the same protocol and the same acceptance
gate --- mean less twice the fold standard deviation above zero, \emph{and} every
fold positive. \best{All three are null.}

Fusing SAM~3's two heads per class rather than by a hardcoded maximum gives
\lvThreePots{} on Potsdam and \lvThreeLove{} on LoveDA. Weighting the presence
score per class gives \lvFour{}. Allowing an additive term as well as a
multiplicative one --- vector scaling in the sense of
Kull~\etal~\cite{kull2019}, the family a reader of the calibration literature
would ask for next --- gives \lvFive{}~$\pm$~\lvFiveSD{}. \best{We
pre-registered each as a small positive and were wrong three times};
\suppref{sec:levers} gives the fits and the diagnosis of each.

\best{The pattern is worth more than any of the three.} The two corrections that
work reshape the \emph{decision} --- which label a score earns, and which class
wins. The three that fail reshape the \emph{evidence} the decision reads. Each
null lever nevertheless recovers the structure it was designed for: the head fit
rediscovers the things/stuff duality with no supervision about which class is
which, and the presence weight orders the classes by how much the gate touches
them at Spearman $\lvGammaRho{}$ with exact $p = \lvGammaP{}$. \best{They identify the right
thing to correct and find that the two working levers have already collected that
mass by another route.} We report them because a method claiming two corrections
is stronger when the neighbouring three are measured rather than left unasked.
...
statistics predict per-class precision at Spearman $\rho \geq \proxyHeadRho{}$ with exact
$p \leq \proxyHeadP{}$, and they still buy nothing. Precision does not determine the
